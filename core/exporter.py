...
from docx import Document  # Используется напрямую, без core.docx_builder
...
def create_docx(blocks, path: Path, title: str = "StageFlow Program"):
    """Генерация DOCX-файла программы."""
    doc = Document()
    doc.add_heading(title, level=1)

    for b in blocks:
        if b.type == "filler":
            line = f"[Тянучка] {b.name}"
        else:
            num = f"{b.num}. " if b.num else ""
            line = f"{num}{b.name}"
